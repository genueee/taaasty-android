...
public abstract class Conversation implements Parcelable {
    ...
    /**
     * ID, проставленный сервером в поле user_id.<br>
     * В приватных чатах — всегда ты (текущий залогиненный пользователь).<br>
     * в публичных — ID, по которому можно определить, от тебя это сообщение или нет. <br>
