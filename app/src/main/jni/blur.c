...
static void stackblurJob(unsigned char* src,         ///< input image data
                  ...
                  unsigned int step                  ///< step of processing (1,2)
                  )
{
    ...
    if (step == 1)
    {
        ...
        for(y = minY; y < maxY; y++)
        {
            ...
            for(x = 0; x < w; x++)
            {
                ...
                dst_ptr[0] = clamp((sum_r * mul_sum) >> shr_sum, 0u, alpha);
                dst_ptr[1] = clamp((sum_g * mul_sum) >> shr_sum, 0u, alpha);
                dst_ptr[2] = clamp((sum_b * mul_sum) >> shr_sum, 0u, alpha);
                dst_ptr += 4;
...
JNIEXPORT void JNICALL Java_ru_taaasty_NdkStackBlur_functionToBlur(JNIEnv* env, jclass clzz __attribute__((__unused__)), jobject bitmapOut, jint radius, jint threadCount, jint threadIndex, jint round) {
    // Properties
    ...
    unsigned int h = infoOut.height;
    unsigned int w = infoOut.width;
